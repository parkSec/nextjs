...
function A(a){a=a.getNextSibling();r.$isElementNode(a)&&(a=a.getFirstDescendant());return null===a||r.$isLineBreakNode(a)||r.$isTextNode(a)&&y.test(a.getTextContent()[0])}function B(a,c,b,d){return(0<a?y.test(b[a-1]):z(d[0]))?c<b.length?y.test(b[c]):A(d[d.length-1]):!1}function C(a,c,b){let d=[],h=[],e=[],f=0,g=0;for(a=[...a];0<a.length;){let l=a[0],n=l.getTextContent().length,q=g;g+n<=c?(d.push(l),f+=n):q>=b?e.push(l):h.push(l);g+=n;a.shift()}return[f,d,h,e]}
function D(a,c,b,d){let h=k.$createAutoLinkNode(d.url,d.attributes);if(1===a.length){var e=a[0];0===c?[g,e]=e.splitText(b):[,g,e]=e.splitText(c,b);var f=r.$createTextNode(d.text);f.setFormat(g.getFormat());f.setDetail(g.getDetail());f.setStyle(g.getStyle());h.append(f);g.replace(h);return e}if(1<a.length){d=a[0];var g=d.getTextContent().length;0===c?e=d:[,e]=d.splitText(c);c=[];for(d=1;d<a.length;d++){let l=a[d],n=l.getTextContent().length,q=g,t=g+n;if(q<b)if(t<=b)c.push(l);else{let [G,H]=l.splitText(b-
q);c.push(G);f=H}g+=n}a=(b=r.$getSelection())?b.getNodes().find(r.$isTextNode):void 0;g=r.$createTextNode(e.getTextContent());g.setFormat(e.getFormat());g.setDetail(e.getDetail());g.setStyle(e.getStyle());h.append(g,...c);a&&a===e&&(r.$isRangeSelection(b)?g.select(b.anchor.offset,b.focus.offset):r.$isNodeSelection(b)&&g.select(0,g.getTextContent().length));e.replace(h);return f}}
function E(a,c,b){var d=[...a];let h=a=d.map(g=>g.getTextContent()).join(""),e,f=0;for(;(e=x(h,c))&&null!==e;){let g=e.index,l=g+e.length;if(B(f+g,f+l,a,d)){let [n,,q,t]=C(d,f+g,f+l);d=(d=D(q,f+g-n,f+l-n,e))?[d,...t]:t;b(e.url,null);f=0}else f+=l;h=h.substring(l)}}
...
function J(a,c,b){u.useEffect(()=>{a.hasNodes([k.AutoLinkNode])||v(77);let d=(h,e)=>{b&&b(h,e)};return p.mergeRegister(a.registerNodeTransform(r.TextNode,h=>{var e=h.getParentOrThrow(),f=h.getPreviousSibling();if(k.$isAutoLinkNode(e)&&!e.getIsUnlinked())F(e,c,d);else if(!k.$isLinkNode(e)){if(h.isSimpleText()&&(y.test(h.getTextContent()[0])||!k.$isAutoLinkNode(f))){e=[h];for(f=h.getNextSibling();null!==f&&r.$isTextNode(f)&&f.isSimpleText();){e.push(f);if(/[\s]/.test(f.getTextContent()))break;f=f.getNextSibling()}E(e,
c,d)}let g=h.getPreviousSibling();e=h.getNextSibling();f=h.getTextContent();!k.$isAutoLinkNode(g)||g.getIsUnlinked()||y.test(f[0])&&!/^\.[a-zA-Z0-9]{1,}/.test(f)||(g.append(h),F(g,c,d),h=g.getURL(),b&&b(null,h));!k.$isAutoLinkNode(e)||e.getIsUnlinked()||y.test(f[f.length-1])||(I(e),F(e,c,d),h=e.getURL(),b&&b(null,h))}}),a.registerCommand(k.TOGGLE_LINK_COMMAND,h=>{let e=r.$getSelection();if(null!==h||!r.$isRangeSelection(e))return!1;e.extract().forEach(f=>{f=f.getParent();if(k.$isAutoLinkNode(f))return f.setIsUnlinked(!f.getIsUnlinked()),
f.markDirty(),!0});return!1},r.COMMAND_PRIORITY_LOW))},[a,c,b])}exports.AutoLinkPlugin=function({matchers:a,onChange:c}){let [b]=m.useLexicalComposerContext();J(b,a,c);return null};exports.createLinkMatcherWithRegExp=function(a,c=b=>b){return b=>{b=a.exec(b);return null===b?null:{index:b.index,length:b[0].length,text:b[0],url:c(b[0])}}}
