    ...
    if (lexical.$isRangeSelection(selection)) {
      if (selection.isCollapsed() && direction === 'backward') {
        ...
        if (lexical.$isRootOrShadowRoot(anchorNode)) {
          ...
          const anchorParentNode = utils.$findMatchingParent(anchorNode, n => lexical.$isElementNode(n) && !n.isInline());
          if (!anchorParentNode) {
            return false;
          }
          const sibling = direction === 'down' ? anchorParentNode.getNextSibling() : anchorParentNode.getPreviousSibling();
